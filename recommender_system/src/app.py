...
def recommend():
    ...
    trip_meetings = [
        meeting for meeting in trip_meetings if 'start' in meeting and 'end' in meeting]
        ...
        for day, categories in nearbyRestaurants.items():
            for category, restaurants in categories.items():  # Iterate over each category within the day
                for restaurant in restaurants:  # Iterate over each restaurant in the category
                    ...
                    restaurant["openingHours"] = convert_opening_hours(
                        restaurant["openingHours"])
                    # Calculate the similarity score for the restaurant based on user interests
                    restaurant["similarity"] = calculate_similarity_score(
                        restaurant, [{'place_similarity': {'types': resto_interests}}], all_types=restoTypeList)
        ...
        for day, categories in nearbyRestaurants.items():
            for category, restaurants in categories.items():  # Iterate over each category within the day
                for restaurant in restaurants:  # Iterate over each restaurant in the category
                    ...
                    restaurant["similarity"] = calculate_similarity_score(
                        restaurant, recentRestaurants, all_types=restoTypeList)
                    print(f'{restaurant['title']} = {restaurant["similarity"]}')

    if len(recent_places) < 5:
        # Use interests
        for place in nearby_places[0]:
            place["regularOpeningHours"] = convert_opening_place_hours(place)
            place["similarity"] = calculate_similarity_score(
                place, [{'place_similarity': {'types': non_resto_interests}}], all_types=place_types)
        ...
        for place in nearby_places[0]:
            place["regularOpeningHours"] = convert_opening_place_hours(place)
            place["similarity"] = calculate_similarity_score(
                place, recent_places, all_types=place_types)
            print(f'{place['displayName']['text']} = {place["similarity"]}')
        scheduled_activities = create_scheduled_activities(
            nearby_places[0], free_slots, trip_meetings, time_zones, nearbyRestaurants)
        return make_response(jsonify({'scheduledActivities': scheduled_activities}), 200)
